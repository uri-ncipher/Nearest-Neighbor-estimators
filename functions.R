  ...
  for (i in 1:n){
    ...
    integrand=function(b){
      ...
      for (j in 1:nrow(subdata)){
        ...
        if (subdata$treatment[j]==1){
          p=p*plogis(as.vector(pred)+as.vector(b))
        } else {p=p*(1-plogis(as.vector(pred)+as.vector(b)))}
      }
      return(p*dnorm(b, mean = 0, sd=theta[length(theta)]))
    }
    prop=c(prop, integrate(integrand, -Inf, Inf)$value)
  }
  return(prop)
}
pi=function(alpha){
...
Y_OE=function(alpha, score){
  ...
  return(mean(data$outcome*(data$treatment)*p0/score)-
           mean(data$outcome*(data$treatment)*p1/score))
}
...
mat=function(theta){
  ...
  propensity_ind_inv=function(theta){
    return(1/propensity_ind(theta))
  }
  U_11=matrix(0, nrow = length(theta), ncol = length(theta))
  v_11=matrix(NA, nrow=n, ncol = length(theta))
  grad_propensity_inv=matrix(NA, nrow = n, ncol = length(theta) )
  for (i in 1:n){
    U_11=U_11-(1/n)*hessian(log_propensity_ind, x=theta)
    v_11[i, ]=grad(log_propensity_ind, x=theta)
    grad_propensity_inv[i, ]=grad(propensity_ind_inv, x=theta)
  }
  
  V_11=matrix(0, nrow = length(theta), ncol =length(theta) )
  for (j in 1:m){
    V_11=V_11+colSums(v_11[data$id[data$component==j],])%*%t(colSums(v_11[data$id[data$component==j],]))
  }
  V_11=V_11*(m/n^2)
  
  return(list(U_11, v_11, V_11, grad_propensity_inv))
...
IPW_1_model=function(data, base_covariate, alpha){
  ...
  L=mat(c(fixef(M), as.data.frame(VarCorr(M))$sdcor))
  APO=rbind(cbind(ldply(alpha, Y_IPW, score=score), alpha=alpha, 
                     type="point estimate"), 
               cbind(ldply(alpha, Var, U_11=L[[1]], v_11=L[[2]], V_11=L[[3]], 
  ...
  CE=rbind(cbind(ldply(alpha, Y_DE, score=score), 
           ...
           cbind(ldply(alpha, Var_DE, U_11=L[[1]], v_11=L[[2]], V_11=L[[3]], 
                      grad_propensity_inv=L[[4]], score=score, theta=theta), 
           ...
           cbind(adply(contrast, 1, Var_IE, U_11=L[[1]], v_11=L[[2]], V_11=L[[3]], 
                       grad_propensity_inv=L[[4]], score=score, theta=theta), 
           ...
           cbind(adply(contrast, 1, Var_TE, U_11=L[[1]], v_11=L[[2]], V_11=L[[3]], 
                       grad_propensity_inv=L[[4]], score=score, theta=theta), 
                 alpha0=contrast[, 1], alpha1=contrast[, 2], type="Var IE")[, -1],
           cbind(adply(contrast, 1, Y_OE, score=score), 
                 alpha0=contrast[, 1], alpha1=contrast[, 2], type="Overall")[, -1],
           cbind(adply(contrast, 1, Var_OE, U_11=L[[1]], v_11=L[[2]], V_11=L[[3]], 
                       grad_propensity_inv=L[[4]], score=score, theta=theta), 
                 alpha0=contrast[, 1], alpha1=contrast[, 2], type="Var IE")[, -1]
           )
...
mat_2=function(theta){
  propensity_2_ind=function(theta){
  ...
  log_propensity_2_ind=function(theta){
    return(log(propensity_2_ind(theta)))
  }
  propensity_2_ind_inv=function(theta){
    return(1/propensity_2_ind(theta))
  }
  
  U_11=matrix(0, nrow = length(theta), ncol = length(theta))
  v_11=matrix(NA, nrow=n, ncol = length(theta))
  grad_propensity_inv_2=matrix(NA, nrow = n, ncol = length(theta) )
  for (i in 1:n){
    U_11=U_11-(1/n)*hessian(log_propensity_2_ind, x=theta)
    v_11[i, ]=grad(log_propensity_2_ind, x=theta)
    grad_propensity_inv_2[i, ]=grad(propensity_2_ind_inv, x=theta)
  }
  ...
  return(list(U_11, v_11, V_11, grad_propensity_inv_2))
}
